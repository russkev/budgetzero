<template lang="html">
  <div>
    <v-row align="center" justify="center">
      <v-col style="position: relative;top: 13%;max-width: 350px;" class="text-xs-center">
        <h3 v-if="this.$store.getters.user.loggedIn">Welcome {{ this.$store.getters.user.email }}!</h3>
        <v-card flat>
          ...
          <v-form>
            ...
            />
            <v-card-actions>
              <v-btn v-if="!this.$store.getters.user.loggedIn" primary large block color="primary" @click.stop="submit">
                Login
              </v-btn>
              <v-btn v-if="this.$store.getters.user.loggedIn" primary large block color="grey" @click.stop="logout">
                Logout
              </v-btn>
            </v-card-actions>
          </v-form>
