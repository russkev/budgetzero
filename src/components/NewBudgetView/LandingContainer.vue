<template>
  <v-overlay opacity="1" z-index="9000" color="background">
    <v-container>
      <div class="landing-page">
        <v-icon icon dark color="primary" size="100">{{ icon }} </v-icon>
        <div class="text-h4 mb-4" style="color: var(--v-primary-base)">{{ title }}</div>
        <router-view></router-view>
        <landing-button v-if="!isLandingPage" label="Back" icon="mdi-arrow-left" :to="{ path: '/landing' }" />
      </div>
    </v-container>
  </v-overlay>
</template>
