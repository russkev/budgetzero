...
describe("NumberRenderer", () => {
  ...
  beforeEach(async () => {
    ...
    await store.dispatch("calculateMonthlyCategoryData");
  ...
  it("renders the correct budget amounts", async () => {
    store.state.month_selected = "2020-12";
    await localVue.nextTick()
    const available_to_budget_last_month = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.available_to_budget_last_month / 100).toFixed(2)
    const available_to_budget_this_month = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.available_to_budget_this_month / 100).toFixed(2)
    const balance_this_month = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.balance_this_month / 100).toFixed(2)
    const budgeted_this_month = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.budgeted_this_month / 100).toFixed(2)
    let income_this_month = financial(store.getters.monthlyCategoryData[store.state.month_selected].summaryData.income_this_month / 100)
    const last_month_overspent = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.last_month_overspent / 100).toFixed(2)
    const overspent = (store.getters.monthlyCategoryData[store.state.month_selected].summaryData.overspent / 100).toFixed(2)
  ...
  //     store.getters.monthlyCategoryData[store.getters.month_selected]["summaryData"]["budgeted_this_month"]
  //   );
  // });
